...
OUTPUT_DIR_DEBUG = ./output/debug
OUTPUT_DIR_RELEASE = ./output/release
...
sgScriptLib: $(PATH_SGSCRIPT)/sgScriptLib.c $(PATH_SGSCRIPT)/sgScriptLib.h sgScriptObj
	...
	$(CC) $(LINKER_FLAGS) $(SGSCRIPT_OBJECTS:%=$(OBJECTS_DIR)/%) -o $(OUTPUT_DIR)/$@.pd_linux
...
sgInputC: $(PATH_SGINPUTC)/sgInputC.c $(PATH_SGINPUTC)/sgInputC.h sgDevice Pack sgInputToFunction
	...
	$(CC) $(LINKER_FLAGS) $(SGINPUTC_OBJECTS:%=$(OBJECTS_DIR)/%) -o $(OUTPUT_DIR)/$@.pd_linux
...
clean:
	rm $(OBJECTS_DIR_DEBUG)/*.o $(OBJECTS_DIR_RELEASE)/*.o $(OUTPUT_DIR_DEBUG)/*.pd_linux $(OUTPUT_DIR_RELEASE)/*.pd_linux
